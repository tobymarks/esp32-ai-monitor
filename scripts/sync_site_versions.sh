...
if [ "$CHECK" = "1" ]; then
  if ! diff -q "$PAGE" "$tmp" >/dev/null; then
    echo "Versionen auf der Seite weichen ab (erwartet: FW v${FW}, App v${APP}):"
  ...
  rm -f "$tmp"
  echo "Seite ist aktuell: FW v${FW}, App v${APP}"
  exit 0
fi
...
echo "Seite und Manifeste aktualisiert: FW v${FW}, App v${APP}"
