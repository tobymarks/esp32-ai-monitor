...
| Stable-Release | **veröffentlicht** | [win-v1.0.0](https://github.com/tobymarks/esp32-ai-monitor/releases/tag/win-v1.0.0) mit `AIMonitor-Setup.exe` (2,9 MB) und `.sha256`, unsigniert. Seite und README stehen auf stable, die Seite ist deployt und verlinkt das Release. Signierung: SignPath-Foundation-Antrag am 12. September 2026 gestellt, am 17. September abgelehnt (zu wenig externe Sichtbarkeit, Wiederbewerbung möglich). Entscheidung 17. September 2026: vorerst unsigniert bleiben, Reichweite aufbauen und später erneut bei SignPath beantragen; Certum bleibt als Rückfall. |
| Zurückstufung | **17. September 2026** | `win-v1.0.0` wieder als Prerelease markiert, Seite, README und Hackster-Bericht nennen Windows „Beta“. Grund: Die App lief nie auf echter Windows-Hardware (Testumgebung aus Phase 0 weiterhin offen: COM-Enumeration, DTR/RTS beim Öffnen, Win-CodexBar mit echten Konten, App-Update, ST7789-Flash). Der Build bleibt derselbe; der Updater findet im Kanal stable damit kein Release und meldet kein Update. Nächster stable-Schritt nach einem Test auf Windows: Tag `win-v1.0.1`. |
| Stable 1.0.1 | **vorbereitet 25. September 2026** | Erster Nutzer im Team hat die App auf echter Windows-Hardware im Einsatz, Grundfunktion bestätigt. Version 1.0.1 filtert informational-Fenster von Win-CodexBar (keine leeren Balken für „No active 5h session“ und Reset Credits) und sendet Zusatz-Credits und Reset Credits als eigene Felder. Seite und README ohne Beta-Hinweis, mit Hinweis auf die fehlende Signatur und die SmartScreen-Warnung. Tag `win-v1.0.1` ohne Prerelease-Flag, damit der Updater im Kanal stable anschlägt. |
